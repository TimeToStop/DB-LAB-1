...
func indexHandler(w http.ResponseWriter, r *http.Request) {
    ...
    rows, err := db.Query("SELECT id, borrowerID, firstName, lastName, surname, passport, inn, snils, driverLicense, additionalDocs, comment FROM individuals")
    if err != nil {
        http.Error(w, err.Error(), http.StatusInternalServerError)
        return
    }
...
func addHandler(w http.ResponseWriter, r *http.Request) {
    if r.Method == http.MethodPost {
        ...
        _, err = db.Exec("INSERT INTO individuals (borrowerID, firstName, lastName, surname, passport, inn, snils, driverLicense, additionalDocs, comment) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
...
func editHandler(w http.ResponseWriter, r *http.Request) {
    if r.Method == http.MethodPost {
        ...
        _, err = db.Exec("UPDATE individuals SET borrowerID=?, firstName=?, lastName=?, surname=?, passport=?, inn=?, snils=?, driverLicense=?, additionalDocs=?, comment=? WHERE id=?",
...
func deleteHandler(w http.ResponseWriter, r *http.Request) {
    ...
    _, err = db.Exec("DELETE FROM individuals WHERE id = ?", id)
    if err != nil {
        http.Error(w, err.Error(), http.StatusInternalServerError)
        return
    }
